...
func TestOVSVersion(t *testing.T) {
	...
	supportsCT, err := ovsif.SupportsConnectionTracking()
	if !supportsCT || err != nil {
		t.Fatalf("Unexpected result: %v, %v", supportsCT, err)
	}
	ensureTestResults(t, fexec)
	...
	addTestResult(t, fexec, "ovs-vsctl --version", "3.4.0", nil)
	supportsCT, err = ovsif.SupportsConnectionTracking()
	if !supportsCT || err != nil {
		t.Fatalf("Unexpected result: %v, %v", supportsCT, err)
	}

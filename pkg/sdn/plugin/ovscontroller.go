...
func (oc *ovsController) SetupOVS(clusterNetworkCIDR, serviceNetworkCIDR, localSubnetCIDR, localSubnetGateway string, masqServices bool) error {
	...
	if !masqServices {
		...
		otx.AddFlow("table=0, priority=350, in_port=2, ip, ct_mark=1, nw_src=%s, nw_dst=%s, actions=ct(nat),goto_table:70", serviceNetworkCIDR, localSubnetCIDR)
	...
	if masqServices {
		...
		otx.AddFlow("table=30, priority=300, ip, nw_dst=%s, ct_state=+trk+rpl, ct_mark=1, actions=ct(nat),goto_table:70", localSubnetCIDR)
